...
_install_dependencies() {
  if [ "$_distro" = "Debian" -o "$_distro" = "Ubuntu" ]; then
    ...
    sudo apt install bc bison build-essential ccache cpio fakeroot flex git kmod libdw-dev libelf-dev libncurses-dev libssl-dev lz4 rsync schedtool wget zstd debhelper llvm clang lld dwarves -y
